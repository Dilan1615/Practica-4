...
import controller.tda.graph.GrafoLableNoDirect;
...
public class HabitacionDao extends AdapterDao<Habitacion> {
    ...
    private GrafoLableNoDirect<String> grafo;
    private LinkedList<String> verticeName;
    private String nombre = "HabitacionGrafo.json";

    public GrafoLableNoDirect<String> crearGrafo() {
        if (verticeName == null) {
        ...
        if (!list.isEmpty()) {
            if (grafo == null) {
                grafo = new GrafoLableNoDirect<>(list.getSize(), String.class);
            }
    ...
    public void guardarGrafo() throws Exception {
        this.grafo.saveGrafoDirigido(nombre);
    }
    ...
    public GrafoLableNoDirect<String> obtenerGrafo() throws Exception {
        if (grafo == null) {
            crearGrafo();
        }
    ...
    public String bfs(int origen) throws Exception {
        ...
        // Llamamos al método de recorrerGrafo() de la clase BFS para obtener el
        // recorrido
    ...
    public JsonObject dataGrafo() throws Exception {
        ...
        if (grafo.existsFile(nombre)) {
            ...
            JsonObject graphData = grafo.getVisGraphData(); // Aquí debe devolver el formato correcto
            System.out.println("Modelo de vis,js " + graphData);
